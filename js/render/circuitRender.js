...
function renderOutput2({ eqs, ffType, nFF, varNames, outVar }) {
  ...
  const AND_X       = MAX_BUS_X + 60;
  const OR_X        = AND_X + 120;
  const FF_INLET    = OR_X + 80;
  ...
  // Mathematical Safe Zones to prevent wire collisions
  const lastFFBot = ffTopY[0] + FF_H;
  const Z_Y = lastFFBot + 160;   // Pushed down to completely clear the bottom CLK route
  const clkY = Z_Y + 100;        // Main CLK trunk sits below the Z logic
  const fbStartY = clkY + 40;    // Feedback loop starts below the CLK trunk
  ...
  // Clock Routing - Uses standard horizontal crossing
  ...
  function createSOPDrawer(isZ) {
      return function drawSOP(ast, targetX, targetY) {
          ...
          function wireFromBus(nm, gInX, gInY) {
              ...
              // Direct horizontal draw to completely avoid overlapping drop lines
