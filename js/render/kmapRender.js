...
// =========================================================
// kmapRender.js — Logic Synthesis Output Renderer
// Renders the "Logic Synthesis" panel: a simplified equations table
// followed by K-map grids with SVG prime-implicant overlays.
// Depends on: logicEngine.js (qm results), styles.css (table classes).
// =========================================================

// Builds and injects the full Logic Synthesis panel HTML into #output1.
// Renders one row per FF input port (color-coded blue) and one row for
// the output variable (color-coded red), then renders all K-maps below.
function renderOutput1({ eqs, groups, ffTT, outTT, ffType, nFF, varNames, outVarNames, states, outVar }) {
  ...
  // One row group per flip-flop bit (MSB first); JK gets two rows per FF
  ...
  // Render one K-map card per equation (FF inputs first, output last)
...
// Generates an HTML K-map for up to 4 variables using Gray code ordering.
// Returns a self-contained HTML string (absolute-positioned table + SVG overlay)
// that can be embedded directly into the equations panel.
//   ones   — minterm indices where f=1
//   dc     — don't-care indices
//   varNames — variable names ordered MSB→LSB
//   groups — prime implicant objects from qm() used to draw colored rectangles
function renderKMap(ones, dc, varNames, groups = []) {
  ...
  // Standard Gray code sequence used for K-map column/row headers
  const gray = [0, 1, 3, 2];

  const HDR_W = 56, CELL_W = 42, CELL_H = 36;
  const borderStyle = "1px solid #94a3b8";

  // Determine grid size based on variable count
  ...
  // Shared inline style strings for header cells
  ...
  // Outer wrapper uses position:relative so the SVG overlay can be absolute-positioned on top
  ...
  // Renders the top-left corner cell with a diagonal divider line (textbook style)
  // showing row variable(s) at bottom-left and column variable(s) at top-right.
  ...
  // Build the table rows according to variable count
  if (n === 1) {
    const [v0] = varNames;
  ...
  // SVG overlay: draw a colored rounded rectangle for each prime implicant group.
  // Staggered insets (4, 6, 8 px) visually separate overlapping groups.
  if (n <= 4 && groups && groups.length > 0) {
    ...
    // Returns contiguous segments for a sorted index array (handles wrap-around pairs)
